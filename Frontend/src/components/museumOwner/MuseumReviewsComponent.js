...
//           {t.museumRating} ({totalMuseumScores} {totalMuseumScores === 1 ? t.review : t.reviews})
//         </h2>
//         <div className="grid grid-cols-1 gap-6">
//           <div className={`${isDarkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-200`}>
//             {renderStars(averageMuseumScore)}
//           </div>
//         </div>
//       </div>
...
//         {reviews && reviews.exhibitions.length > 0 ? (
//           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//             {reviews.exhibitions.map((exhibition, index) => {
//               const { exhibitionScores } = exhibition;
//               const totalExhibitionScores = exhibitionScores.length;
//               const averageExhibitionScore = totalExhibitionScores
//                 ? (exhibitionScores.reduce((sum, score) => sum + score, 0) / totalExhibitionScores).toFixed(2)
//                 : "N/A";

//               return (
//                 <div
//                   key={`${exhibition.exhibitionId._id || exhibition.exhibitionId}-${index}`}
//                   dir={language === "he" ? "rtl" : "ltr"}
//                   className={`${isDarkMode ? "bg-gray-800" : "bg-white"} rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow duration-200`}
//                 >
//                   <h3 className={`text-md font-semibold ${isDarkMode ? "text-gray-100" : "text-gray-800"} mb-4`}>
//                     {exhibition.exhibitionId.name.charAt(0).toUpperCase() + exhibition.exhibitionId.name.slice(1)}{" "}
//                     ({totalExhibitionScores} {totalExhibitionScores === 1 ? t.review : t.reviews})
//                   </h3>
//                   {renderStars(averageExhibitionScore)}
//                 </div>
//               );
...
//           </div>
//         ) : (
//           <div className={`${isDarkMode ? "bg-gray-800" : "bg-gray-50"} rounded-lg p-8 text-center ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
//             <p className="text-lg">{t.noReviews}</p>
//           </div>
//         )}
//       </div>
...
const MuseumReviewsComponent = ({ museumId }) => {
  ...
  const renderStars = (score) => {
    ...
    return (
      <div className="flex items-center space-x-1">
        {[...Array(5)].map((_, index) => {
          const fillPercentage = Math.max(0, Math.min(1, numericScore - index));
          return (
