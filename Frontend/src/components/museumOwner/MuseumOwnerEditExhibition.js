...
const MuseumOwnerEditExhibition = () => {
    ...
    const { fields: newCurators, append: appendNewCurator, remove: removeNewCurator } = useFieldArray({
      control,
      name: "newCurators",
    });
    ...
    const onSubmit = async (data) => {
      ...
      const updatedData = {
        ...
        newCurators: newCurators.filter(curator => curator.name && curator.email) // Exclude empty new curator fields
      };
      try {
    ...
    return (
      <div>
        ...
        {loading ? (
          ...
          <form id="exhibitionForm" onSubmit={handleSubmit(onSubmit)} className="max-w mx-auto shadow p-6">
            <div className="mb-4">
            ...
            <div className="flex justify-end mb-4">
              <button
                type="button"
                onClick={() => appendNewCurator({ name: "", lastName: "", email: "", phoneNumber: "" })}
                ...
                {newCurators.map((field, index) => (
                  <tr key={field.id} className="even:bg-gray-100">
                    <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                      <input
                        type="text"
                        {...register(`newCurators.${index}.name`, { required: "Curator name is required" })}
                        className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${errors.newCurators?.[index]?.name ? 'border-red-500' : ''}`}
                      />
                      {errors.newCurators?.[index]?.name && <span className="text-red-500">{errors.newCurators[index]?.name?.message}</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                      <input
                        type="text"
                        {...register(`newCurators.${index}.lastName`, { required: "Curator lastName is required" })}
                        className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${errors.newCurators?.[index]?.lastName ? 'border-red-500' : ''}`}
                      />
                      {errors.newCurators?.[index]?.lastName && <span className="text-red-500">{errors.newCurators[index]?.lastName?.message}</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                      <input
                        type="email"
                        ...
                        className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${errors.newCurators?.[index]?.email ? 'border-red-500' : ''}`}
                      />
                      {errors.newCurators?.[index]?.email && <span className="text-red-500">{errors.newCurators[index]?.email?.message}</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                      <input
                        type="tel"
                        {...register(`newCurators.${index}.phoneNumber`, { required: "Curator phone number is required" })}
                        className={`mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 ${errors.newCurators?.[index]?.phoneNumber ? 'border-red-500' : ''}`}
                      />
                      {errors.newCurators?.[index]?.phoneNumber && <span className="text-red-500">{errors.newCurators[index]?.phoneNumber?.message}</span>}
                    </td>
                    <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200 flex justify-center items-center space-x-2">
                      <button
                        type="button"
