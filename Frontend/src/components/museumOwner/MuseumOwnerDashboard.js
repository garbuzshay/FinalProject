    ...
    if (!isLoading && museum && planDetails && exhibitions.length > 0) {
      ...
      const lineData = {
        ...
        datasets: [
          {
            label: translations[language].exhibitionsCreationTrend,
            data: sortedDates.map((date) => newCreationTrend[date].length),
            backgroundColor: barColors.slice(0, sortedDates.length), // Use the same bar colors
            hoverBackgroundColor: barColors.slice(0, sortedDates.length),
          },
        ],
      };
    ...
    >
      ...
      {chartData.barData && (
        ...
        >
          ...
          <div className="h-64">
            <Bar data={chartData.lineData} options={lineChartOptions} />
          </div>
        </div>
      )}
        {chartData.pieData && (
