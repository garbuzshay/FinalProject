...
const AdminTermsPricesPackages = () => {
    const [terms, setTerms] = useState('These are the initial terms and conditions...');
        ...
        <div className="p-4">
            ...
            {isEditingTerms ? (
                <form onSubmit={handleSaveTerms} className="mb-5">
                    <div className="mb-4">
                        <label className="block text-gray-700 mb-2" htmlFor="terms">Edit Terms and Conditions</label>
                        <textarea
                    ...
                    <button className="w-full bg-blue-500 text-white px-4 py-2 rounded" type="submit">Save Terms</button>
                </form>
            ) : (
                <div>
            ...
            {isAdding ? (
                <form onSubmit={handleAddPlan} className="mb-5">
                    <div className="mb-4">
                        <label className="block text-gray-700 mb-2" htmlFor="name">Package Name</label>
                        <input
                    ...
                    <div className="mb-4">
                        <label className="block text-gray-700 mb-2" htmlFor="maxExhibitions">Max Exhibitions</label>
                        <input
                            className="w-full px-2 py-1 border rounded"
                            type="number"
                            id="maxExhibitions"
                            value={newPlan.maxExhibitions}
                            onChange={(e) => setNewPlan({ ...newPlan, maxExhibitions: e.target.value })}
                        />
                    </div>
                    <div className="mb-4">
                        <label className="block text-gray-700 mb-2" htmlFor="maxArtWorks">Max Artworks</label>
                        <input
                            className="w-full px-2 py-1 border rounded"
                            type="number"
                            id="maxArtWorks"
                            value={newPlan.maxArtWorks}
                            onChange={(e) => setNewPlan({ ...newPlan, maxArtWorks: e.target.value })}
                        />
                    </div>
                    <div className="mb-4">
                    ...
                    <button className="w-full bg-green-500 text-white px-4 py-2 rounded" type="submit">Add Plan</button>
                    <button 
                        ...
                        <React.Fragment key={index}>
                            {editingPlanIndex === index ? (
                                <tr>
                                    ...
                                    <td className="border px-4 py-2">
                                        <input
                                            ...
                                            onChange={(e) => setNewPlanData({ ...newPlanData, maxExhibitions: e.target.value })}
                                        />
                                    </td>
                                    <td className="border px-4 py-2">
                                        <input
                                            ...
                                            onChange={(e) => setNewPlanData({ ...newPlanData, maxArtWorks: e.target.value })}
                                        />
                                    </td>
                                    <td className="border px-4 py-2">
                                        <input
                                ...
                                <tr className="cursor-pointer" onClick={() => handleEditPlan(index)}>
                                    ...
                                    <td className="border px-4 py-2">{plan.maxExhibitions === null ? 'Unlimited exhibitions' : `Up to ${plan.maxExhibitions} exhibitions`}</td>
                                    <td className="border px-4 py-2">{plan.maxArtWorks === null ? 'Unlimited artworks' : `Manage up to ${plan.maxArtWorks} artworks`}</td>
