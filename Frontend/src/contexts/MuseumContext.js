...
export const MuseumProvider = ({ children }) => {
  const { museum, isLoading, error, fetchMuseum, updateExhibition  } = useUserMuseum();
  const planDetails = usePlanDetails(museum);

  return (
    <MuseumContext.Provider value={{ museum, isLoading, error, fetchMuseum ,updateExhibition }}>
