...
const ExhibitionPage = () => {
  ...
  const { museum, exhibitions } = useMuseum(); // Access museum and exhibitions from context
  ...
  // Find the exhibition from context using exhibitionId
  const exhibition = exhibitions?.find(
    (exhibition) => exhibition._id === exhibitionId
  );

  // Display artworks loading spinner for 1 second on initial load
  useEffect(() => {
    const timer = setTimeout(() => setLoadingArtworks(false), 350);
  ...
  // Artwork list and filtered list for display
  const artworksToDisplay = exhibition.artworks || [];
  const artworksToRender = searchQuery ? filteredArtworks : artworksToDisplay;
  ...
  const handleSearch = (e) => {
    ...
    const filtered = artworksToDisplay.filter((artwork) =>
      artwork.title.toLowerCase().includes(query)
    );
  ...
  const handleArtworkClick = (indexInFiltered) => {
    ...
    setSelectedArtworkIndex(indexInAllArtworks); // Set selected artwork
  };
